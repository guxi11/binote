...
```bash
git clone https://github.com/Guxi11/binote.git
cd binote
npm install
npm run build
```
...
src/                    # TypeScript source → compiles to dist/
  ...
  core/
    ...
    note-io.ts          # Read/write .binote/ notes
    binote-paths.ts   # Path conventions, link resolution, config
...
2. **Slash commands** (`commands/*.md`) — prompt-based skills for Claude Code. These orchestrate the MCP tools with behavioral rules (e.g. "read binotes before source files").
...
```markdown
---
description: "One-line description. Triggers on: '/binote:name'."
