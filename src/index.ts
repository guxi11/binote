...
server.registerTool(
  "read_note",
  {
    description: "Read one or more Binotes. Path is relative to .binote/ (e.g. 'src/index.ts.md'). Use notePaths for batch reads, from/to for line ranges.",
    inputSchema: {
...
server.registerTool(
  "write_note",
  {
    description: "Create or update a Binote. Use [[filename]] or [[path/to/file]] for bidirectional links.",
    inputSchema: {
      projectRoot: z.string().describe("Absolute path to the project root"),
...
server.registerTool(
  "search",
  {
    description: "Full-text search across all Binotes. Hits include resolved [[link]] targets on the matched line — use those instead of a follow-up query_links.",
    inputSchema: {
      projectRoot: z.string().describe("Absolute path to the project root"),
