...
const commands: Record<string, (args: readonly string[]) => Promise<void>> = {
  ...
  async read(args) {
    ...
    if (positional.length === 0) {
      console.error("Usage: binote read <notePath> [<notePath>...] [--from N] [--to M] [--lines N:M] [--root D]");
      process.exit(1);
    }
    const config = makeConfig(rootFromFlags(flags));
  ...
  async write(args) {
    ...
    if (!notePath || contentArg === undefined) {
      console.error("Usage: binote write <notePath> <content> [--root D]");
      process.exit(1);
    }
    const config = makeConfig(rootFromFlags(flags));
  ...
  async links(args) {
    ...
    if (!notePath) {
      console.error("Usage: binote links <notePath> [--detail] [--root D]");
      process.exit(1);
    }
    const config = makeConfig(rootFromFlags(flags));
  ...
  async search(args) {
    ...
    if (!query) {
      console.error("Usage: binote search <query> [--regex] [--max N] [--context N] [--root D]");
      process.exit(1);
    }
    const config = makeConfig(rootFromFlags(flags));
  ...
  async resolve(args) {
    ...
    if (!target) {
      console.error("Usage: binote resolve <target> [--root D]");
      process.exit(1);
    }
    const config = makeConfig(rootFromFlags(flags));
    const notes = await scanExistingNotes(config);
...
Usage: binote <command> [args]
       binote [--help|-h] [--version|-v]

Commands:
  init     [--root D]                          Initialize .binote/ from project structure
